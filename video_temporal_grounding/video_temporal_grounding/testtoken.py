...
class DynamicActionMemory:
    ...
    def get_memory_prompt(self, action_class, current_time):
        """
        生成记忆Prompt：纯FIFO展示，无时间衰减、无置信度衰减
        """
        ...
        for m in list(self.action_memories[action_class]):
            ...
            history_segments.append(
                ...
                f"Act:{action_token}, Conf:{m['conf']:.2f}]"
            )
        
        return " -> ".join(history_segments) if history_segments else "No recent history."
...
class CharadesVideoGrounding:
    def __init__(self, model_path, video_dir, annotation_dir, device_map="auto",
        ...
        # FIFO记忆库，无衰减
        self.memory_manager = DynamicActionMemory(memory_size=5)
        ...
        print(f"🎯 记忆策略：纯FIFO（无衰减）| Token提取：7个核心Token")
...
"""
    ...
    def process_single_video(self, video_id):
        ...
        print(f"🔄 记忆策略：纯FIFO（无任何置信度/时间衰减）")
        ...
        for idx, window in enumerate(windows):
            ...
            try:
                ...
                for action in actions_to_detect:
                    ...
                    state_tag = f"[STATE:{state.upper()}]"
                    print(f"   - {action_class}: {status} {state_tag} | 置信度: {confidence:.3f}")
    ...
    def process_batch(self, split='test', max_videos=None, output_file='charades_grounding_batch_fifo.json'):
        """批量处理视频"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Charades视频动作定位 (FIFO无衰减消融实验)')
    ...
    parser.add_argument('--output_file', type=str, default='charades_grounding_batch_fifo.json', help='批量结果输出文件')
