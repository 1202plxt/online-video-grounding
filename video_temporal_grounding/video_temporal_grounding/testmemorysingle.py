...
def extract_key_tokens_from_explanation(explanation, action_class, action_desc, state="none"):
    ...
    if action_desc_words:
        action_token = action_desc_words[0][:15]
    else:
...
"""
    ...
    # ================= 核心修改点：加入 target_index =================
    def process_batch(self, split='test', target_index=None, max_videos=None, output_file='charades_grounding_batch_7token.json'):
        """批量处理视频（支持指定索引单视频处理）"""
        ...
        if target_index is not None:
            if 0 <= target_index < len(filtered_video_ids):
                ...
                filtered_video_ids = [selected_video_id]
        ...
        print(f"\n====================================================")
        print(f"🚀 开始处理视频 (带记忆增强架构)")
        print(f"📁 数据集划分: {split} | 原始数量: {len(all_video_ids)}")
        print(f"✅ 有效视频数量: {len(filtered_video_ids) if target_index is None else '(本次仅处理指定1个)'}")
...
def main():
    ...
    parser.add_argument('--max_videos', type=int, default=None, help='批量处理的最大视频数（若指定 target_index 则忽略）')
    parser.add_argument('--output_file', type=str, default='charades_grounding_batch_7token.json', help='结果输出文件')
