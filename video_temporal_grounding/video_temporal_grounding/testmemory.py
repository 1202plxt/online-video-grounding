...
# ======================== 【消融实验】仅保留 STATE 状态Token，其余全部删除 ========================
def extract_key_tokens_from_explanation(explanation, action_class, action_desc, state="none"):
    """
    🔥 消融实验：仅保留 state 状态 token
    输出格式保持 7 段结构不变，但全部填充为 state
    不影响任何流程、不改变代码结构、不影响输出格式
    """
    # 只保留 state，其余全部替换为 state（保持输出格式长度不变，不破坏代码）
    return f"{state}_{state}_{state}_{state}_{state}_{state}_{state}"

# ======================== 动态历史记忆库模块（仅保留 state 衰减机制，结构完全不变） ========================
class DynamicActionMemory:
    def __init__(self, memory_size=5, time_horizon=10.0):
        self.action_memories = {}
        self.memory_size = memory_size
        self.time_horizon = time_horizon
        # 非对称衰减配置（仅保留 state 衰减）
        self.decay_config = {
            "start": 0.75,     # 起始状态衰减慢
        ...
        for m in list(self.action_memories[action_class]):
            ...
            if decayed_conf >= self.min_conf_threshold:
                history_segments.append(
                    ...
                    f"Vis:{m['state']}/{m['state']}, "
                    f"Temp:{m['state']}/{m['state']}, "
                    f"Act:{m['state']}, Eff.Conf:{decayed_conf:.2f}]"
                )
...
class CharadesVideoGrounding:
    def __init__(self, model_path, video_dir, annotation_dir, device_map="auto",
        ...
        print(f"🎯 消融实验：仅保留 STATE 状态Token + 非对称衰减")
...
"""
    ...
    def process_single_video(self, video_id):
        ...
        if not actions_to_detect:
            print(f"⚠️  视频 {video_id} 无标注动作，创建空动作占位")
            actions_to_detect = [{'class': f'empty_action_{i+1}', 'start': 0.0, 'end': duration, 'desc': 'Empty action placeholder'} 
                                for i in range(self.max_action_num)]
            select_note = "无标注动作，创建占位动作"
        ...
        print(f"🔄 非对称衰减配置: start=0.85 | continue=0.80 | end=0.35 | none=0.30")
        ...
        final_result = {
            ...
            "processing_info": {
                ...
                "tiou_thresholds": self.tiou_thresholds,
                "token_config": "ABLATION: ONLY STATE TOKEN"
            },
    ...
    def process_batch(self, split='test', max_videos=None, output_file='charades_grounding_batch_state_only.json'):
        """批量处理视频"""
        ...
        print(f"\n====================================================")
        print(f"🚀 开始批量处理视频")
        print(f"📁 数据集划分: {split} | 处理数量: {len(all_video_ids)}")
        print(f"🎯 tIoU评估阈值: {self.tiou_thresholds}")
        print(f"🔄 非对称衰减配置: start=0.85 | continue=0.80 | end=0.35 | none=0.30")
        print(f"🎨 消融配置：仅保留 STATE 状态Token")
        print(f"====================================================")
        
        batch_results = []
        for i, video_id in enumerate(all_video_ids):
            print(f"\n🔢 批量进度: {i+1}/{len(all_video_ids)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Charades视频动作定位 (消融-仅STATE)')
    ...
    parser.add_argument('--output_file', type=str, default='charades_grounding_batch_state_only.json', help='批量结果输出文件')
