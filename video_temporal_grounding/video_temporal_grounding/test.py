...
class CharadesVideoGrounding:
    def __init__(self, model_path, video_dir, annotation_dir, device_map="auto",
                 window_size=1, stride=1, num_frames=12, conf_threshold=0.7, max_action_num=5,
...
"""
    ...
    def process_single_video(self, video_id):
        ...
        if not all_actions or len(all_actions) == 0:
            print(f"⏭️  视频 {video_id}: 无动作标签，跳过处理")
            return None
    ...
    # ================= 核心修改点：支持 target_index =================
    def process_batch(self, split='test', target_index=None, max_videos=None, output_file='charades_grounding_batch_baseline.json'):
        ...
        if target_index is not None:
            if 0 <= target_index < len(filtered_video_ids):
                selected_video_id = filtered_video_ids[target_index]
                filtered_video_ids = [selected_video_id]  # 将处理列表缩小到仅包含这一个视频
                print(f"🎯 已指定处理第 {target_index + 1} 个有效视频，Video ID: {selected_video_id}")
            else:
                print(f"❌ 指定的索引超出有效视频范围 (有效视频总数: {len(filtered_video_ids)})")
                return None
        elif max_videos:
            filtered_video_ids = filtered_video_ids[:max_videos]

        print(f"\n====================================================")
        print(f"🚀 开始批量处理视频 (BASELINE 无记忆版本 | 对齐记忆版评估格式)")
        print(f"📁 数据集划分: {split} | 原始总数: {len(all_video_ids)}")
        print(f"✅ 有效视频总数: {len(filtered_video_ids) if target_index is None else '(本次仅处理指定1个)'}")
        print(f"====================================================")

        batch_results = []
        for i, video_id in enumerate(filtered_video_ids):
            # 这里的 i+1 只是循环计数，并非全局索引
            print(f"\n🔢 运行进度: {i+1}/{len(filtered_video_ids)}")
            res = self.process_single_video(video_id)
            if res:
                batch_results.append(res)

        print(f"\n====================================================")
        print(f"📊 开始评估 (共处理 {len(batch_results)} 个视频)")
        print(f"====================================================")

        batch_eval = evaluate_batch_complete(batch_results, self.tiou_thresholds)

        print(f"\n🎯 BASELINE 评估结果 (对齐记忆版输出):")
...
def main():
    ...
    parser.add_argument('--max_videos', type=int, default=None, help='最大处理视频数（若指定 target_index 则忽略）')
    
    # 新增参数：指定具体的索引
    parser.add_argument('--target_index', type=int, default=19, help='指定处理过滤后的第几个视频(从0开始，19即第20个)')
    ...
    # 将 target_index 传入 process_batch
